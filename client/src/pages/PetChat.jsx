...
export default function PetChat() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                const petRes = await fetch(`localhost:5000/api/pets/${petId}`);
                if (petRes.ok) setPet(await petRes.json());

                const userRes = await fetch(`localhost:5000/api/users/${otherUserId}/public`);
        ...
        const fetchMessages = async () => {
            try {
                const chatRes = await fetch(`localhost:5000/api/messages/${petId}/${otherUserId}`, {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            const res = await fetch('localhost:5000/api/messages/send', {
                method: 'POST',
                headers: { 
