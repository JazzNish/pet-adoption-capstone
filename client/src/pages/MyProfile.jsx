...
export default function MyProfile() {
    ...
    useEffect(() => {
        const fetchMyData = async () => {
            ...
            try {
                const userRes = await fetch(`http://localhost:5000/api/users/${localUser.id}/public`);
                if (userRes.ok) {
                ...
                const petsRes = await fetch(`http://localhost:5000/api/pets/my-pets/${localUser.id}`);
                if (petsRes.ok) {
